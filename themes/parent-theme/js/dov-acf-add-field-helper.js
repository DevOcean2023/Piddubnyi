...
( function ($) {
	...
	function changeFieldLabel($field) {
		let value = $field.find('[data-name="label"] input[type="text"]').val();
		...
		if ('button' === value) {
			...
			value = window.confirm('Is WYSIWYG') ? 'wysiwyg' : 'textarea';
		} else if ('all' === value) {
			value = 'true_false';
		} else if ('posts' === value) {
			value = 'relationship';
	...
	function changeFieldType($field) {
		...
		let delay;
		const value = $field
			.find('[data-name="label"] input[type="text"]')
			.val();
		...
		switch (type) {
			...
			case 'textarea':
				// eslint-disable-next-line no-alert
				rows = window.prompt('How many rows?', '4');
				// eslint-disable-next-line no-alert
				newLines = window.confirm('Automatically add paragraphs?')
					? 'wpautop'
					: 'br';

				$field.find('[data-key="rows"] input[type="number"]').val(rows);
			...
			case 'relationship':
				$field
					.find(
						'[data-key="filters"] input[type="checkbox"][value!="search"]'
					)
					.prop('checked', false);
