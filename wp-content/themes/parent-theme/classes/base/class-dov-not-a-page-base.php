...
class DOV_Not_A_Page_Base {
	public static function init() : void {
		acf_add_local_field_group(
			array(
				'key'                   => 'not_a_page',
				'title'                 => __( 'Not a Page', 'theme' ),
				'fields'                => array(
					array(
						'key'               => 'not_a_page_redirect_type',
						'label'             => __( 'Redirect Type', 'theme' ),
						'name'              => 'redirect_type',
						'type'              => 'button_group',
						'instructions'      => '',
						'required'          => 0,
						...
						'choices'           => array(
							'first-child' => __( 'First Child', 'theme' ),
							'url'         => __( 'URL', 'theme' ),
						),
						'allow_null'        => 0,
		...
		add_filter( 'theme_templates', array( static::class, 'add_template' ), 10, 4 );
		add_filter( 'template_redirect', array( static::class, 'redirect' ) );
		add_filter( 'display_post_states', array( static::class, 'post_states' ), 10, 2 );
	}
	...
	public static function add_template( array $post_templates, WP_Theme $wp_theme, ?WP_Post $post, string $post_type
		...
		if ( 'page' === $post_type ) {
			$post_templates['not-a-page'] = __( 'Not a Page', 'theme' );
		}
	...
	public static function redirect() : void {
		if ( ! is_search() && 'not-a-page' === get_page_template_slug() ) {
			$redirect = '';
	...
	public static function post_states( array $post_states, WP_Post $post ) : array {
		if ( 'not-a-page' === get_page_template_slug( $post->ID ) ) {
			$post_states['not-a-page'] = __( 'Not a Page', 'theme' );
		}
