...
class DOV_ACF_Forms_Field_Base extends acf_field {
	...
	public function set_forms() : void {
		if ( class_exists( 'GFFormsModel' ) ) {
			...
			$wpdb->dov_gf_forms = GFFormsModel::get_form_table_name();
			if ( GFCommon::table_exists( $wpdb->dov_gf_forms ) ) {
				$results = $wpdb->get_results( "SELECT id, title FROM $wpdb->dov_gf_forms WHERE is_trash = 0" );
				if ( $results ) {
