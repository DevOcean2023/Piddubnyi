...
class DOV_ACF_Flex_Content_Base {
	public static string|false $id = false;

	public static bool $change_title = true;
	...
	public static function init() : void {
		add_filter( 'acf/load_field/type=flexible_content', array( static::class, 'set_layouts' ) );
		add_filter( 'body_class', array( static::class, 'body_class' ) );
		add_filter( 'acf/get_field_group', array( static::class, 'change_title' ) );
	}
	...
	public static function set_layouts( array $field ) : array {
		...
		$display        = apply_filters( 'dov_flex_display', 'row' );
		...
		if ( isset( $field['wrapper']['id'] ) && 'tpl_flexible_content_content' === $field['wrapper']['id'] ) {
			...
			$layouts = array();
			if ( $groups ) {
				remove_filter( 'acf/load_field/type=flexible_content', array( static::class, __METHOD__ ) );
				...
				add_filter( 'acf/load_field/type=flexible_content', array( static::class, __METHOD__ ) );
			}
			$field['layouts'] = static::set_inner_layouts( $layouts );
		}
	...
	public static function body_class( array $classes ) : array {
		...
		return apply_filters( 'dov_flex_body_class', $classes );
	}

	public static function get_the_title( string $title, int $level, string $class = '' ) : string {
		_deprecated_function( __METHOD__, '5.1.3', 'ACF functions or DOV_Fields' );
	...
	public static function the_content( string $slug = 'template-parts/flexible-content/', string $layout = '' ) : void {
		if ( empty( $layout ) ) {
			$layout = (string) get_row_layout();
		}

		if ( empty( static::$blocks ) ) {
			static::$blocks = get_field( 'content', false, false );
		}

		static::$index ++;

		DOV_BAM_Content::set_block_name( $layout );
		...
		if ( static::$id ) {
			$id = ' id="' . static::$id . '"';
		} elseif ( false !== static::$id ) {
			$id = ' id="section-' . static::get_index() . '"';
		} else {
			$id = false;
		}
		...
		$content = implode( ' data-visibility="' . $layout . '">', explode( '>', $content, 2 ) );

		$next    = static::$blocks[ static::get_index() ] ?? array();
		$classes = apply_filters( 'dov_flex_block_class', array(), $next['acf_fc_layout'] ?? '' );
		if ( $classes ) {
		...
		$items    = apply_filters( 'dov_flex_replace_items', 'left|right|item' );
		$wrappers = apply_filters(
			'dov_flex_replace_wrappers',
