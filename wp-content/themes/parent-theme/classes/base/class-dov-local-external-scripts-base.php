...
class DOV_Local_External_Scripts_Base {
	public const CRON_HOOK = 'dov_local_external_scripts';
		...
		if ( $local_paths ) {
			foreach ( $local_paths as $local_path ) {
				...
				if ( file_exists( $info_path ) ) {
					$info = DOV_Filesystem::get_file_contents( $info_path );
					if ( $info ) {
					...
					DOV_Filesystem::delete_file_or_folder( $info_path );
				}

				DOV_Filesystem::delete_file_or_folder( $local_path );
			}
		}
	}
	...
	protected static function get_dir() : string {
		...
		if ( null === $dir ) {
			$dir = wp_upload_dir()['basedir'] . '/dov-externals/';
		}
	...
	protected static function get_url() : string {
		...
		if ( null === $dir ) {
			$dir = wp_upload_dir()['baseurl'] . '/dov-externals/';
		}
