...
class DOV_CPT_Base {
	public const KEY = 'dov_cpt_slug_';
	...
	public static function init() : void {
		...
		add_filter( 'dov_thumbnails_support', array( static::class, 'set_thumbnail_support' ) );
	}
	...
	public static function get_single_label( string $post_type ) {
		...
		return apply_filters( 'dov_get_cpt_label_single', $label, $post_type );
	}
	...
	public static function get_plural_label( string $singular, string $post_type ) {
		...
		return apply_filters( 'dov_get_cpt_label_plural', $label, $post_type );
	}
		...
		foreach ( static::$permalinks as $post_type => $permalink ) {
			...
			add_settings_field(
				...
				static function () use ( $id, $post_type ) {
					...
					$home = home_url( '/' );
					$tags = array();
					if ( class_exists( 'DOV_Tax' ) ) {
						$tags = DOV_Tax::get_tags_by_post_type( $post_type );
