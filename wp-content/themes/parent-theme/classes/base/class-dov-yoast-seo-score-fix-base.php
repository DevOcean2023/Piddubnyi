...
class DOV_Yoast_SEO_Score_Fix_Base {
	public static function init() : void {
		if ( DOV_Theme::$yoast_enable ) {
			add_filter(
				'wpseo_pre_analysis_post_content',
				array( static::class, 'pre_analysis_post_content' ),
				10,
				2
			);
			...
			if ( is_admin() ) {
				add_action(
					...
					array( static::class, 'js' )
				);
			}

			if ( wp_doing_ajax() ) {
				add_action(
					'wp_ajax_dov_get_content_by_yoast_seo_score',
					array( static::class, 'ajax_get_content_by_yoast_seo_score' )
				);
			}

			add_action(
				'after_delete_post',
				array( static::class, 'delete_post' )
			);
		}
	}
		...
		if ( 'templates/tpl-flexible-content.php' === get_page_template_slug() ) {
			...
			<script>
				jQuery(window).on('YoastSEO:ready', function () {
					const pluginName = 'dovFlexContentPlugin';
					...
					jQuery.post({
						url: ajaxurl,
						data: {
							action: 'dov_get_content_by_yoast_seo_score',
							post_id: '<?php echo esc_js( $post_id ); ?>',
							_ajax_nonce: '<?php echo esc_js( wp_create_nonce( 'dov_get_content_by_yoast_seo_score' ) ); ?>'
						}
	...
	public static function ajax_get_content_by_yoast_seo_score() : void {
		check_ajax_referer( 'dov_get_content_by_yoast_seo_score' );

		$post_id = $_POST['post_id'] ?? 0;
		if ( $post_id ) {
			$post = get_post( $post_id );
			if ( $post ) {
				$content = static::get_content( $post );
	...
	public static function get_content( ?WP_Post $current_post ) : string {
		...
		if ( $post_id && empty( $content[ $post_id ] ) ) {
			...
			if ( have_rows( 'content', $current_post ) ) {
				DOV_ACF_Title_Field::$has_main_auto_title = true;
