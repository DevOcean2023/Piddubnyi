...
class DOV_List_Table_Base extends WP_List_Table {
	public const TITLE      = '';
	public const TABLE      = '';
	public const SLUG       = '';
	...
	public static DOV_List_Table_Base $table;

	public static string $screen_id    = '';
	public static string $page_title   = '';
	public static string $page_content = '';

	public static int $single_id = 0;

	public static array $single_data = array();
	...
	public static function save_screen_option( bool $keep, string $option, string $value ) : bool|string {
	...
	public static function load() : void {
		// todo: Check if we can add a nonce, if not write why.
	...
	public function prepare_items() : void {
		...
		if ( $wpdb->last_error ) {
			$this->items = array();
			DOV_Admin_Notices::add_notice( $wpdb->last_error, 'error' );
		} else {
			$this->items = $result;
	...
	public static function process_action( string $view ) : void {
		...
		if ( $is_edit || $is_add ) {
			...
			if ( $is_edit ) {
				if ( 0 === $id ) {
					DOV_Admin_Notices::add_notice( 'Error empty ' . static::ID_COLUMN, 'error' );
				...
				if ( $is_single ) {
					...
					foreach ( static::REL_TABLES as $table_data ) {
						...
						// todo: Think maybe you can escape IN, well, or at least $id.
						// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
						$wpdb->query( $query );
						if ( $wpdb->last_error ) {
							DOV_Admin_Notices::add_notice( $wpdb->last_error, 'error' );
			...
			if ( $wpdb->last_error ) {
				...
				DOV_Admin_Notices::add_notice( $is_edit ? 'Updated' : 'Created', 'success' );
	...
	public static function get_back_to_table() : string {
		...
		if ( 'table' === $view ) {
			...
			// todo: Check if we can add a nonce, if not write why.
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$referer = wp_validate_redirect( $_GET['back_to_table'] ?? '' );
			if ( $referer ) {
	...
	public static function set_single_data( string $view ) : void {
		...
		$id = absint( $_GET[ static::ID_COLUMN ] ?? '' );
		if ( 0 === $id ) {
			DOV_Admin_Notices::add_notice( 'Error ' . static::ID_COLUMN, 'error' );
		...
		// todo: Check Escape SQL.
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		static::$single_data = $wpdb->get_row( $wpdb->prepare( $query, $id ), ARRAY_A );
		static::$single_id   = $id;
		...
		foreach ( static::REL_TABLES as $table_data ) {
			...
			// todo: Check Escape SQL.
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$data = $wpdb->get_results( $wpdb->prepare( $query, $id ), ARRAY_A );
	...
	public static function set_page_title( string $view ) : void {
		if ( 'single' === $view ) {
			static::$page_title = esc_html__( 'View', 'theme' );
		} elseif ( 'edit' === $view ) {
			static::$page_title = esc_html__( 'Edit', 'theme' );
		} elseif ( 'add' === $view ) {
			static::$page_title = esc_html__( 'Add', 'theme' );
		} else {
	...
	public static function the_input( string $name, array $field, string $value ) : void {
		...
		// todo: We need to do attribute escaping, we have a function from ACF, but it's better to write your own.
	...
	public static function back_button() : void {
		...
		if ( $back_url ) {
			...
			echo esc_html__( 'Back to Table', 'theme' );
			echo '</a>';
		}
	}
	...
	public static function edit_view() : void {
		...
		<form method="post">
			...
			submit_button( __( 'Save', 'theme' ), 'primary', 'submit', false );
			static::back_button();
				...
				<table class="wp-list-table widefat fixed">
					...
					<tr>
						...
						foreach ( $fields as $name => $field ) {
							...
							$label = $field['label'] ?? '';
							echo '<th scope="col" class="' . esc_attr( $primary ) . '">' . esc_html( $label ) . '</th>';
							$primary = '';
						}
						?>
						<th><?php esc_html_e( 'Actions', 'theme' ); ?></th>
					</tr>
					</thead>
					<tbody>
					<?php
	...
	public static function the_rel_table_row( array $row, array $table_data, bool $tpl = false ) : void {
		...
		foreach ( $row as $name => $value ) {
			...
			echo '<td class="' . esc_attr( $primary ) . '" data-colname="' . esc_attr( $label ) . '" data-required="' . esc_attr( $required ) . '">';
			...
			if ( $primary ) {
				...
				<button type="button" class="toggle-row">
					<span class="screen-reader-text">
						<?php esc_html_e( 'Show more details', 'theme' ); ?>
					</span>
				</button>
		...
		<td>
			<button type="button" class="button button-small button-trash">
				...
				<span class="screen-reader-text"><?php esc_html_e( 'Trash', 'theme' ); ?></span>
			</button>
			<button type="button" class="button button-small button-move">
				<span class="dashicons dashicons-move"></span>
				<span class="screen-reader-text"><?php esc_html_e( 'Move', 'theme' ); ?></span>
			</button>
		</td>
		<?php
	...
	public static function add_view() : void {
		...
		<form method="post">
			...
			submit_button( __( 'Add', 'theme' ), 'primary', 'submit', false );
			static::back_button();
	...
	public static function hidden_inputs() : void {
		...
		echo '<input type="hidden" name="page" value="' . esc_attr( static::get_slug() ) . '">';
	}
	...
	public static function page() : void {
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php echo esc_html( static::$page_title ); ?></h1>
			<a href="<?php echo esc_url( static::get_view_url( 'add' ) ); ?>" class="page-title-action">
				<?php esc_html_e( 'Add New', 'theme' ); ?>
			</a>
			<hr class="wp-header-end">
			<?php do_action( 'all_admin_notices' ); ?>
			<?php
			// todo: think about how we can escape output ... although it's probably not possible :(
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo static::$page_content;
			?>
		</div>
		<?php
	}
	...
	protected function get_sortable_columns() : array {
		...
		foreach ( static::FIELDS as $key => $field ) {
			if ( isset( $field['order'] ) ) {
				$columns[ $key ] = array( $key, mb_strtolower( $field['order'] ) === 'asc' );
			}
		}
	...
	protected function handle_row_actions( $item, $column_name, $primary ) : string {
		...
		$actions           = array();
		$actions['view']   = '<a href="' . $single_url . '">' . __( 'View', 'theme' ) . '</a>';
		$actions['edit']   = '<a href="' . $edit_url . '">' . __( 'Edit', 'theme' ) . '</a>';
		$actions['delete'] = '<a href="' . $delete_url . '">' . __( 'Delete', 'theme' ) . '</a>';
