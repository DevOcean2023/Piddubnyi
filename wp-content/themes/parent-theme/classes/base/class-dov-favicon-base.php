...
class DOV_Favicon_Base {
	public static function init() : void {
		remove_action( 'wp_head', 'wp_site_icon', 99 );
		add_filter(
			'acf/update_value/name=dov_other_favicon',
	...
	public static function save_favicon( $value ) {
		...
		if ( $favicon ) {
			...
			update_option(
				'dov_other_favicon_html',
				sprintf( '<link rel="shortcut icon" href="%s">', esc_url( $favicon ) ),
				true
			);
		} else {
			delete_option( 'dov_other_favicon_html' );
		}
	...
	public static function the_favicon() : void {
		echo wp_kses(
			get_option( 'dov_other_favicon_html' ),
			array(
				'link' => array(
					'rel'  => true,
					'href' => true,
