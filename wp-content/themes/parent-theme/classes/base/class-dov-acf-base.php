...
class DOV_ACF_Base {
	public static function init() : void {
		...
		add_action(
			'acf/init',
			array( static::class, 'fixed_shortcode_wpautop' )
		);
		add_filter(
			'acf_the_content',
		...
		add_filter(
			'acf_the_content',
			array( DOV_Images::class, 'remove_image_wrap_p' ),
			99
		);
		add_action(
			'acf_the_content',
			array( DOV_BAM_Content::class, 'set_classes' ),
			100
		);
		add_filter(
			'acf_the_content',
			array( DOV_TinyMCE::class, 'delete_paragraphs' ),
			-1
		);
	}
	...
	public static function register_fields() : void {
		...
		acf_register_field_type( 'DOV_ACF_Replace_Text_Field' );
		acf_register_field_type( 'DOV_ACF_Contact_Link_Field' );
		acf_register_field_type( 'DOV_ACF_Copyright_Field' );
		acf_register_field_type( 'DOV_ACF_Menu_Field' );
		acf_register_field_type( 'DOV_ACF_Social_Links_Field' );
		acf_register_field_type( 'DOV_ACF_Title_Field' );
		acf_register_field_type( 'DOV_ACF_Background_Field' );

		if ( DOV_Theme::$gf_enabled ) {
			acf_register_field_type( 'DOV_ACF_Forms_Field' );
		}
	}
	...
	public static function get_custom_choices( $field ) {
		if ( isset( $field['allow_custom'] ) && 1 === $field['allow_custom'] && ! acf_is_screen( 'acf-field-group' ) ) {
			$choices          = get_option( 'dov_custom_choices_' . $field['key'] );
	...
	public static function save_custom_choices( $values, $post_id, $field ) {
		if ( isset( $field['allow_custom'] ) && 1 === $field['allow_custom'] && is_array( $values ) ) {
			...
			if ( $update ) {
				update_option( 'dov_custom_choices_' . $field['key'], $choices, false );
			}
		}
	...
	public static function replace_trailing_slash( string $html ) : string {
		return preg_replace( '/<((img|hr|br)([^>]*))\/>/', '<$1>', $html );
	}
}
