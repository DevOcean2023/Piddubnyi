...
class DOV_BAM_Menu_Base {
	...
	public static function sub_menu_class( array $classes, stdClass $args, int $depth ) : array {
		if ( static::check( $args ) ) {
			...
			if ( 1 === $depth ) {
				$classes[] = $args->bam_block_name . '__sub-items_level_top';
			} else {
				$classes[] = $args->bam_block_name . '__sub-items_level_sub';
			}
		}
	...
	public static function item_class( array $classes, WP_Post $menu_item, stdClass $args, int $depth ) : array {
		if ( static::check( $args ) ) {
			...
			if ( $menu_item->has_children ) {
				$bam_classes[] = $args->bam_block_name . '__item_has_sub-menu';
			}

			if ( $menu_item->top ) {
				$bam_classes[] = $args->bam_block_name . '__item_level_top';
				if ( $menu_item->top_first ) {
					$bam_classes[] = $args->bam_block_name . '__item_top_first';
				}

				if ( $menu_item->top_last ) {
					$bam_classes[] = $args->bam_block_name . '__item_top_last';
				}
			} else {
				$bam_classes[] = $args->bam_block_name . '__item_level_sub';
			}
	...
	public static function link_class( array $atts, WP_Post $menu_item, stdClass $args, int $depth ) : array {
		if ( static::check( $args ) ) {
			...
			$bam_classes[] = $args->bam_block_name . '__link';
			$bam_classes[] = $args->bam_block_name . '__link_level_' . ( ++$depth );
			...
			if ( $is_current ) {
				$bam_classes[] = $args->bam_block_name . '__link_current';
			}

			if ( $is_ancestor ) {
				$bam_classes[] = $args->bam_block_name . '__link_ancestor';
			}

			if ( $menu_item->has_children ) {
				$bam_classes[] = $args->bam_block_name . '__link_has-sub-menu';
			}

			if ( $menu_item->top ) {
				$bam_classes[] = $args->bam_block_name . '__link_level_top';
				if ( $menu_item->top_first ) {
					$bam_classes[] = $args->bam_block_name . '__link_top_first';
				}

				if ( $menu_item->top_last ) {
					$bam_classes[] = $args->bam_block_name . '__link_top_last';
				}
			} else {
				$bam_classes[] = $args->bam_block_name . '__link_level_sub';
			}
			...
			if ( $classes ) {
				foreach ( $classes as $class ) {
					if ( preg_match_all( '/\b(?=\w)_[\w-]+/m', $class, $matches, PREG_SET_ORDER ) ) {
						foreach ( $matches as $match ) {
							$bam_classes[] = $args->bam_block_name . '__link' . $match[0];
						}
					} else {
						$bam_classes[] = $class;
					}
			...
			$atts['class'] = implode( ' ', $bam_classes );
		}
	...
	public static function add_has_children( array $items, object $args ) : array {
		if ( ! empty( $args->bam_block_name ) ) {
			...
			if ( $top_items ) {
				$top_items[0]->top_first    = true;
				end( $top_items )->top_last = true;
			}
		}
