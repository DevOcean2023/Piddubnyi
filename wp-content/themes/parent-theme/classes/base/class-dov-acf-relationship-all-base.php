...
class DOV_ACF_Relationship_All_Base {
	...
	public static function settings( $field ) : void {
		acf_render_field_setting(
			$field,
			array(
				'label' => __( 'Enable All Checkbox', 'theme' ),
				'name'  => static::KEY,
				'type'  => 'true_false',
				'ui'    => 1,
			)
	...
	public static function the_checkbox( $field ) : void {
		if ( static::enabled( $field ) ) {
			...
			$label = esc_html__( 'All', 'theme' );

			/** @noinspection HtmlUnknownAttribute */
			printf(
				'<p%s><label><input type="checkbox" class="dov-r-all" value="1" name="%s" %s>%s</label></p>',
	...
	public static function load_value( $value, $post_id, array $field ) {
		if ( static::enabled( $field ) && ! is_admin() ) {
			...
			if ( 'yes' === $all ) {
				...
				$value = get_posts( apply_filters( 'dov_relationship_all_args', $args, $field ) );
			}
		}
	...
	public static function script() : void {
		...
		<script>
			jQuery(function ($) {
				$('.dov-r-all').on('change', toggle).each(toggle);
	...
	public static function is_all( string $selector ) : bool {
		$all   = 'no';
		$field = DOV_Fields::get_field( $selector );
		if ( $field && static::enabled( $field ) ) {
			$name = static::get_name( $field );
			$all  = get_post_meta( DOV_Theme::get_post_id_taking_into_preview(), $name, true );
		}
