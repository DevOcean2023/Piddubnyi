...
class DOV_Log_Base {
	...
	protected static bool $has_changes = false;

	protected static array $logs            = array();
	protected static array $ignore_levels   = array();
	protected static array $ignore_messages = array();
	protected static array $ignore_paths    = array();
	...
	public static function write( WP_Error|string $wp_error_or_message, string $type = 'error', string $handle = 'dov' ) : void {
	...
	public static function action_shutdown() : void {
		...
		if ( static::$has_changes ) {
			foreach ( static::$logs as $handle => $contents ) {
				DOV_Filesystem::put_file_contents( static::get_log_path( $handle ), $contents );
			}
		}
	}
	...
	public static function error_handler( int $level, string $message, $file = null, $line = null, $context = null ) : bool {
		if (
			...
			$type = match ( $level ) {
				E_WARNING, E_USER_WARNING, E_STRICT, E_DEPRECATED, E_USER_DEPRECATED => 'warning',
		...
		if ( class_exists( 'QM_Collectors' ) && static::is_need_qm_log() ) {
			/**
	...
	protected static function get_log( $handle ) : string {
		if ( ! isset( static::$logs[ $handle ] ) ) {
			static::$logs[ $handle ] = DOV_Filesystem::get_file_contents( static::get_log_path( $handle ) );
		}
	...
	protected static function get_log_dir() : string {
		...
		if ( null === $dir ) {
			$dir = wp_upload_dir()['basedir'] . '/dov-logs/';
		}

		return $dir;
	}

	protected static function get_log_name( string $handle = 'dov' ) : string {
		static $names = array();
	...
	protected static function has_duplicate( string $message, string $handle = 'dov' ) : bool {
		...
		$has = str_contains( static::get_log( $handle ), $message );
		if ( $has ) {
