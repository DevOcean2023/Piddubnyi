...
class DOV_XLSX_CSV_Base {
	public static string $path = '';

	public static function to_array( string $file_path, string $file_name ) {
		$data_file_path = static::get_extract_path( md5_file( $file_path ) );
		...
		if ( file_exists( $data_file_path ) ) {
			...
			if ( $check['ext'] ) {
				if ( 'csv' === $check['ext'] ) {
					$rows = static::csv_to_array( $file_path );
				} else {
					$rows = static::xlsx_to_array( $file_path );
				}
			} else {
				return new WP_Error( 'invalid_ext', 'Invalid Ext' );
			}

			DOV_Filesystem::put_file_contents(
				$data_file_path,
	...
	public static function csv_to_array( string $file_path ) : array {
		...
		if ( false !== $file ) {
			while ( false !== ( $raw_row = fgetcsv( $file ) ) ) {  // phpcs:ignore
				static::set_row( $raw_row, $rows, $index, $max_cell );
			}
	...
	protected static function set_row( array $raw_row, array &$rows, int &$index, int &$max_cell ) : void {
		...
		if ( $test ) {
			...
			foreach ( $raw_row as $cell => $cell_value ) {
				$row[ $cell ] = static::fix_utf8_non_breaking( mb_convert_encoding( $cell_value, 'UTF-8' ) );
			}
	...
	public static function xlsx_to_csv( string $file_path, int $sheet_id = 1 ) : bool {
		$csv_dir = static::get_path( pathinfo( $file_path, PATHINFO_FILENAME ) );
		$rows    = static::xlsx_to_array( $file_path, $sheet_id, $sheet_name );

		static::mkdir( $csv_dir );
		...
		foreach ( $rows as $row ) {
			static::file_put_csv( $csv_open, $row );
		}
	...
	public static function get_path( string $filename = '' ) : string {
		if ( empty( static::$path ) ) {
			static::$path = wp_get_upload_dir()['basedir'] . '/dov-importer/';
		}
	...
	public static function xlsx_to_array( string $file_path, int $sheet_id = 1, string &$sheet_name = '' ) : array {
		static::xlsx_unzip( $file_path );

		$strings    = static::get_strings();
		$sheets     = static::get_sheets();
		$numbers    = range( '0', '9' );
		$rows       = array();
		$xml_path   = static::get_extract_path( 'xl/worksheets/sheet' . $sheet_id . '.xml' );
		...
		while ( 'row' === $xml_reader->name ) {
			...
			$xml_array   = static::xml_to_array( $xml_element );
		...
		return static::normalize_rows( $rows, $max_cell );
	}
	...
	protected static function xlsx_unzip( string $file_path ) : void {
		...
		if ( ! file_exists( static::get_extract_path( $md5 ) ) ) {
			static::clear();

			static::mkdir( static::get_extract_path() );

			file_put_contents( static::get_extract_path( $md5 ), '' ); // phpcs:ignore

			$archive = new PclZip( $file_path );

			$archive->extract( PCLZIP_OPT_PATH, static::get_extract_path() );
		}
	}
	...
	protected static function get_strings() : array {
		$file_name = static::get_extract_path( 'strings.json' );
		if ( file_exists( $file_name ) ) {
			return json_decode( file_get_contents( $file_name ), true ); // phpcs:ignore
		}

		$xml_path   = static::get_extract_path( 'xl/sharedStrings.xml' );
		$strings    = array();
		$xml_reader = new XMLReader();
		...
		// phpcs:ignore
		file_put_contents( static::get_extract_path( 'strings.json' ), wp_json_encode( $strings ) );
		...
		if ( is_object( $xml_element ) ) {
			foreach ( $namespace as $ns => $ns_url ) {
				...
				foreach ( $_children as $child_name => $child ) {
					...
					$children[ $child_name ][] = static::xml_to_array( $child );
				}
			}
		}
	...
	protected static function get_sheets( string $file_path = '' ) : array {
		...
		$xml_path   = static::get_extract_path( 'xl/workbook.xml' );
		$sheets     = array();
		$xml_reader = new XMLReader();
		$index      = 1;
